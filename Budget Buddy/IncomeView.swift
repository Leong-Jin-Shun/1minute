        ...
        GeometryReader { proxy in
            VStack {
                ...
                ZStack {
                    ...
                    VStack {
                        Text("You receive a daily income of around").font(.custom("Christmas School", size: 20)).frame(width: 300).lineSpacing(1.5).multilineTextAlignment(.center).offset(y: 62.5).shadow(color: .white, radius: 3.5).offset(y: -(proxy.size.height / 13.5) - 5)
                        
                        Spacer()
                        
                        Text("$\(totalIncome, specifier: "%.2f")").font(.custom("AniTypewriter", size: 50)).offset(y: -(proxy.size.height / 11.5))
